    ...
    try {
      ...
      if (admin.apps.length === 0) {
        ...
        if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
          ...
          // Mode développement sans Firebase - ne pas bloquer l'application
        ...
        this.messaging = admin.messaging();
        this.isEnabled = true;
        console.log('✅ Service de notifications push Firebase déjà initialisé');
      }
    } catch (error) {
  ...
  async sendToUsers(userIds, notification) {
    ...
    try {
      ...
      if (validUsers.length === 0) {
        console.log('⚠️ Aucun utilisateur valide trouvé pour les notifications');
