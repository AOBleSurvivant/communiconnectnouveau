...
const AlertsPage = () => {
  ...
  const handleAlertUpdate = async (updatedAlert) => {
    try {
      console.log('Mise à jour de l\'alerte:', updatedAlert);
      ...
      {loading ? (
        ...
        <Grid container spacing={3}>
          {filteredAlerts.filter(alert => alert && alert.title).map((alert) => (
            <Grid item xs={12} sm={6} md={4} key={`alert-${alert._id}-${alert.createdAt || Date.now()}-${alert.title}`}>
